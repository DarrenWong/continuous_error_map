...
class ActorFactory(object):
    def __init__(self, world: carla.World, client: carla.Client, base_save_dir=None):
        self._uid_count = 0
        self.world = world
        self.client = client
    ...
    def create_vehicle_node(self, actor_info):
        ...
        path = actor_info["path"]
        print(path)
        route=[]
        for ind in path:
          route.append(self.spawn_points[ind].location)
        print(type(vehicle_object))
        print(type(vehicle_object.vehicle_agent))
        print(type(vehicle_object.carla_actor))

        print(route)

        tm.set_path(vehicle_object.carla_actor, route)
